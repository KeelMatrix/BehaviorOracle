...
$scratchParent = if ($env:PAPERCLIP_RUN_SCRATCH_DIR) {
    $env:PAPERCLIP_RUN_SCRATCH_DIR
} else {
    [System.IO.Path]::GetTempPath()
}
$scratch = Join-Path $scratchParent "behaviororacle-real-targets-$([Guid]::NewGuid().ToString('N'))"
...
function Get-DivergenceStats {
    ...
    if ($divergences.Count -eq 0) {
        return [pscustomobject]@{
            ...
            ReviewerReadable = 'not-applicable-no-stable-divergences'
        }
    }
    ...
    return [pscustomobject]@{
        ...
        ReviewerReadable = 'pending-independent-review'
    }
}
