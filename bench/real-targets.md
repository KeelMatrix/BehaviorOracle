...
This is the reproducible real-library Phase 0 probe for the bounded BehaviorOracle semantic domain. It compares pinned published package assets from two versions of each target. It does not claim arbitrary semantic equivalence, and it does not infer precision or recall for published-version pairs without a ground-truth label.
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| Humanizer.Core | `EQUIVALENT_WITHIN_TESTED_DOMAIN` | 392 / 414 | 392 | 25 | 6.38% | 25 | 32 / 32 | 0 | 0 | 367 | 863.4 ms | byte-identical |
| Newtonsoft.Json | `EQUIVALENT_WITHIN_TESTED_DOMAIN` | 687 / 687 | 687 | 1 | 0.15% | 1 | 32 / 32 | 0 | 0 | 686 | 902.2 ms | byte-identical |
| Npgsql | `NONDETERMINISTIC_INCONCLUSIVE` | 838 / 839 | 838 | 32 | 3.82% | 32 | 32 / 29 | 0 | 3 | 806 | 953.9 ms | byte-identical |
...
| Bounded runtime suitable for ordinary CI | **PASS** | Synthetic median comparison: 822.9 ms for 80 scenarios; real-target per-scenario medians: 863.4–953.9 ms, with bounded end-to-end JSON runs of about 27.5–33.3 s per 32-scenario target. |
...
The mandatory Phase 0 probe passes for the explicitly bounded semantic domain, with the real-library-value criterion passed narrowly and conservatively. The durable implementation may be retained for independent review inside this domain; no breadth claim, hosted execution claim, or release authorization follows from this evidence. The low real-world supported percentages, lack of ground-truth real divergences, and lack of a real minimization witness remain documented residual limitations.
