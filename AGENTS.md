...
The benchmark intentionally reports its planted divergence with exit code `1`; it is enabled only by `bench/Run-Benchmark.ps1` and is not a shipped CLI command. The package smoke expects an equivalent comparison to exit `0` and a planted divergence to exit `1`. The real-library subset is part of the canonical local CI-equivalent path: it uses the three pinned targets in `bench/real-targets/targets.json`, `config.json` bounds of 32 scenarios and three confirmation runs, a five-minute comparison bound, bounded output capture, repeated byte-identical JSON checks, and committed stable-evidence checks. `Test-RealLibraryBenchmark.ps1` verifies the pins/bounds and that an over-budget configuration fails closed. `action/Test-Action.ps1` validates the Action wrapper on Windows PowerShell/.NET 8, including path handling, failure propagation, shallow-checkout missing-ref failure, the documented full-history/tag checkout workflow, and cleanup. CI runs the full test, format, benchmark, dependency-audit, and telemetry-suppression checks on `windows-latest`, `ubuntu-latest`, and `macos-latest`; the Windows matrix leg also runs the Action validation. A separate Ubuntu job fails closed when advisory data is unavailable, and the dependent Ubuntu `package` job inspects the exact archives and runs the isolated consumer smoke.
...
Run the focused test class first, then the full test project, Release build, format verification, synthetic benchmark, bounded pinned real-library subset, package contract inspection, isolated package-consumer smoke, and required dependency audit. Run `action/Test-Action.ps1` on Windows. The real-library subset remains network-dependent, but it is part of the canonical local CI-equivalent path and fails closed when its inputs, comparisons, or reproducibility checks fail. CI repeats repository validation on Windows, Linux, and macOS runners, runs the required fail-closed audit on Ubuntu, and performs package inspection and consumer smoke on Ubuntu. Record missing platform or network evidence rather than inferring it.
